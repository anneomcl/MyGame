...
{
	PlayerCharacter * player = (PlayerCharacter *)_gameObjectManager.getByType("PlayerCharacter");
...
//Maybe redirect to main menu instead of playing new game?
...
void Game::start(void)
{
	...
	initObject(new PlayerCharacter, sf::Vector2f(9 * SCREEN_WIDTH / 10, 1000), "PlayerCharacter");

	Background * bg = new Background("C:/Users/Anne/Documents/Visual Studio 2013/Projects/MyGame/Graphics/Background_condensed.png");
...
void Game::displayCoins()
{
	if (display_coin)
	{
		...
		{
			display_coin = false;
			Coin * coin = (Coin *)_gameObjectManager.getByType("Coin");
...
void Game::updatePlayerCoins(RigidSurface_Mystery * block)
{
	PlayerCharacter * player = (PlayerCharacter *)_gameObjectManager.get("PlayerCharacter");
	...
	initObject(new_coin,
		sf::Vector2f(block->getSprite().getGlobalBounds().left, block->getSprite().getGlobalBounds().top - 100)
...
void Game::handleHitBlock(RigidSurface_Mystery * block)
{
	PlayerCharacter * player = (PlayerCharacter *)_gameObjectManager.get("PlayerCharacter");

	if (block->times_hit < 3)
	{
		updatePlayerCoins(block);
	}
	else
	{
		//block is dead
...
void Game::handleSurfaces()
{
	...
	for (int i = 0; i < rigidBodyCoords.size(); ++i)
	{
		if (player->getSprite().getGlobalBounds().intersects(rigidBodyCoords[i]->getSprite().getGlobalBounds()))
		{
			...
			{
				handleHitBlock((RigidSurface_Mystery *) rigidBodyCoords[i]);
			}
...
void Game::handleSurfaceMovement()
{
	PlayerCharacter * player = (PlayerCharacter *)_gameObjectManager.get("PlayerCharacter");
	if (player->velocity.y > 0)
	{
...
std::vector<VisibleGameObject *> Game::findRigidBodies()
{
	...
	std::vector<VisibleGameObject *> surfaces_vector = _gameObjectManager.getAllByType("RigidSurface");
